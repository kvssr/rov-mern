...
const addRaid = async (
  ...
  const raid = await prisma.raid.create({
    data: {
      ...
      total_kills: squad_data["total"]["kills"],
      total_deaths: squad_data["total"]["deaths"],
      raidTypeId: raidType,
    },
  });
  return raid;
...
const addCharacterRaidInfo = async (raid_id, character_id, data) => {
  const character_raid_info = await prisma.characterRaidInfo.create({
    data: {
      ...
      duration_in_combat: Math.max(data["duration_present"].dmg_players, -1),
      normalization_time_allies: Math.max(
        data["normalization_time_allies"].dmg_players,
        -1
      ),
      num_fights_present: data.num_fights_present.dmg_players,
